...
export const FollowingsUser = ({
  ...
  useLayoutEffect(() => {
    if (followingUser.followers) {
      setIsFollowing(followingUser.followers.includes(user._id))
    }
    if (user._id == followingUser._id) {
      setIsSelf(true)
    } else {
      setIsSelf(false)
    }
  ...
  const handleClick = async () => {
    if (!isFollowing) {
      try {
        //users.js（ No.5 ）
        await axios.put(
          `${process.env.NEXT_PUBLIC_PUBLIC_SERVER}/api/users/${followingUser._id}/follow`,
          user
        )
      } catch (err) {
        console.log(err)
      ...
      try {
        //users.js（ No.5 ）
        await axios.put(
          `${process.env.NEXT_PUBLIC_PUBLIC_SERVER}/api/users/${followingUser._id}/unfollow`,
          user
        )
      } catch (err) {
        console.log(err)
      ...
      <Link href={`/profile/${followingUser._id}`}>
        <a className="flex items-center text-inherit no-underline hover:underline">
          <div className="m-0 flex w-32 items-center p-0">
            <Avatar />
            <p className="ml-2">{followingUser.userName}</p>
          </div>
        </a>
      </Link>
      {!isSelf &&
